...
class TutorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        True
        """
        for i in range(3): # remove three cards from opponent's hand
            tmp = opponent.play(0)
        
        for i in range(3): # draw three cards
            opponent.draw()
...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        ATK, DEF = other_card.attack, other_card.defense
        
        # add value to player's card
        for x in player.deck.cards:
            x.attack += ATK
            x.defense += DEF
        
        # delete opponent's cards that matches the value
        op_deck_copy = opponent.deck.cards[:]
        opponent.deck.cards = [x for x in op_deck_copy if x.attack != ATK and x.defense != DEF]
