...
class Insect:
    ...
    def place_locator(self, beehive):
        st_place, end_place = self.place, beehive
        # determine distance
        # 1. min_range situation
        if self.min_range != -1:
            min_dis = self.min_range
            while st_place != beehive and min_dis != 0:
                st_place = st_place.entrance
                min_dis -= 1
            
            # return cur_place, beehive
        
        # 2. max_range situation
        if self.max_range != float('inf'):
            end_place = self.place
            max_dis = self.max_range
            while end_place != beehive and max_dis != 0:
                end_place = end_place.entrance
                max_dis -= 1
            
            # return self.place, cur_place.entrance
        
        # 3. other situations(avoid bug)
        
        return st_place, end_place.entrance
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        # setting starting place and end place
        st_place, end_place = self.place_locator(beehive)
        
        # finding bees in that range
        while st_place != end_place:
            if len(st_place.bees) != 0:
                return rANTdom_else_none(st_place.bees)
            st_place = st_place.entrance
        return None
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # judge die or not
        damage = amount + self.damage if self.armor <= 0 else amount
        # attack bees in the same place
...
class ScubaThrower(ThrowerAnt):
    """Water_safe Version of ThrowerAnt"""
    name = 'Scuba'
    implemented = True
    food_cost = 6
    is_watersafe = True
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    # BEGIN Problem EC
    implemented = True  # Change to True to view in the GUI
    times_deployed = 0  # avoid multiple QueenAnt
    ...
    def __init__(self, armor=1):
        ...
        if QueenAnt.times_deployed == 0:
            self.real_check = True
            QueenAnt.times_deployed += 1
        else:
            self.real_check = False
        # END Problem EC
    
    def action(self, gamestate):
        """A queen ant throwzs a leaf, but also doubles the damage of ants
        in her tunnel.
        ...
        # BEGIN Problem EC
        # real queen
        if self.real_check == True:
            # 1. throw things
            ScubaThrower.action(self, gamestate)
            # 2. double other ants' damage
            cur_place = self.place.exit
            while cur_place != None:
                if cur_place.ant != None and cur_place.ant.double_damage == False:
                    cur_place.ant.damage *= 2
                    cur_place.ant.double_damage = True
    ...
    def reduce_armor(self, amount):
        ...
        """
        # BEGIN Problem EC
        if self.real_check == True and amount >= 1:
            bees_win()
        else:
