...
def swine_align(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4a
    "*** YOUR CODE HERE ***"
    # 1. special case(one score is 0)
    if player_score == 0 or opponent_score == 0:
        return False

    # 2. normal case(performing GCD)
    gcd = 1
...
def play_process(who, strategy, player_score, opponent_score, dice):
    """
    the process during the round for a single person
    also with extra_turn judgment
    """
    roll_num = strategy(player_score, opponent_score)
    player_score += take_turn(roll_num, opponent_score, dice)
