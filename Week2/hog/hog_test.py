...
def play(
    ...
    def play_extra_turn(strategy, player, player_score, opponent_score, say=silence):
        ...
        while extra_turn(new_player_score, opponent_score):
            new_player_score += take_turn(
                strategy(new_player_score, opponent_score), opponent_score, dice
            )
            # print("DEBUG: extra turn activated.")

            if (
                player == 0
            ):  # Get the score0, score1, new_player_score, and opponent_score inserted right
        ...
        return (
            new_player_score,
            say,
        )  # Return the score after extra turns and the say function
...
def final_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    extra_turn_threshold = (
        75  # The probability that worth to trust on RNG for an extra turn
    )
    extra_turn_rolls, extra_turn_rolls_efficiency = best_to_extra_turn(
    ...
    if score >= sprint_threshold:  # Last moments strategy
        ...
        if free_bacon_extra_turn_efficiency(
            score, opponent_score
        ):  # If free bacon could bring an extra turn
            num_rolls = 0
        elif (
            free_bacon(opponent_score) >= cutoff
        ):  # If free bacon is worth for the points
            num_rolls = 0
        elif (
            extra_turn_rolls_efficiency >= extra_turn_threshold
        ):  # If an extra turn is worth trying
    ...
    #     num_rolls = 10 # Pray for pure luck, or we can follow the back of our opponent
        ...
        if free_bacon_extra_turn_efficiency(
            ...
            free_bacon(opponent_score) >= cutoff - 1
        ):  # If free bacon is worth for the points
            num_rolls = 0
        elif (
            extra_turn_rolls_efficiency >= extra_turn_threshold
